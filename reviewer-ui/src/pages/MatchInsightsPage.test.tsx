...
describe('MatchInsightsPage empty-state messaging', () => {
  it('highlights when no samples have been captured', async () => {
    ...
    expect(screen.getAllByText('Match statistics are not available yet.').length).toBeGreaterThan(0);
    expect(
      screen.getByText('Match statistics are not available for this mapping yet.'),
    ).toBeInTheDocument();
  });
});
